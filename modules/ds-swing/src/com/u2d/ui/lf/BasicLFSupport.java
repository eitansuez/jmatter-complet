...
public class BasicLFSupport implements LookAndFeelSupport
{
   ...
   public static class JGoodiesLooks implements LFProvider
   {
      ...
      public LookAndFeel[] lfs()
      {
         if (_lfs == null)
         {
            _lfs = new LookAndFeel[clsNames.length];
            for (int i=0; i<clsNames.length; i++)
            {
               try
               {
                  Class lfClass = Class.forName(clsNames[i]);
                  _lfs[i] = (LookAndFeel) lfClass.newInstance();
               }
   ...
   public static class SystemLFProvider implements LFProvider
   {
      ...
      public LookAndFeel[] lfs()
      {
         if (_lfs == null)
         {
            ...
            _lfs = new LookAndFeel[lfinfo.length];
            for (int i=0; i<lfinfo.length; i++)
            {
               _lfs[i] = lookAndFeel(lfinfo[i].getClassName());
            }
         }
         return _lfs;
      }
      private LookAndFeel lookAndFeel(String clsName)
      {
         try
         {
